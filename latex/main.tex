\documentclass[a4paper,12pt]{article}

\input{etc/preamble}

%%% Всю шаблонную информацию можно менять тут
\newcommand{\FullCourseNameFirstPart}{\so{АРХИТЕКТУРА КОМПЬЮТЕРОВ И}}
\newcommand{\FullCourseNameSecondPart}{\so{ОПЕРАЦИОННЫЕ СИСТЕМЫ}}
\newcommand{\SchoolName}{ФПМИ}
\newcommand{\TrackName}{ПМИ Информатика}
\newcommand{\SemesterNumber}{III}

\newcommand{\CourseName}{АКОС} %  Используется в преамбуле для создания названия предмета в верхнем контитуле   
\newcommand{\CourseDate}{2022}           %  Используется в преамбуле для создания даты в нижнем контитуле

%\includeonly{lectures/lect06.tex}  % Чтобы скомпилировать только часть лекций

\begin{document}
    \input{etc/title_page}
    \newpage
    \hypertarget{intro}{}
    \tableofcontents

    \input{lectures/lect01.tex}
    \input{lectures/lect02.tex}
    \input{lectures/lect03.tex}
    \input{lectures/lect04.tex}
    \input{lectures/lect05.tex}
    \input{lectures/lect06.tex}
    \input{lectures/lect07.tex}
    \input{lectures/lect08.tex}
    \input{lectures/lect09.tex}
    \input{lectures/lect10.tex}
    \input{lectures/lect11.tex}
    
\end{document}